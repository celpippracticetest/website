...
async function main() {
  ...
  const profile = await sql`
    SELECT plan, plan_type, public_metadata
    FROM public.user_profiles
    WHERE supabase_auth_user_id = 'f7948895-fb4a-41a1-8bf3-d94563c37580'::uuid
  `;
  console.log(JSON.stringify(profile, null, 2));

  // Also find similar stale users: plus + planCancelled + expired, or plus with canceled stripe
  const stale = await sql`
    SELECT p.email, p.plan, p.plan_type,
    ...
    LIMIT 30
  `;
  console.log("similar stale count", stale.length);
