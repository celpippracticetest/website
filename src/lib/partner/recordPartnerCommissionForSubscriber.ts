...
/**
 * After the `users` document reflects a purchase, credit the partner (if any) once per subscriber.
 * Does not call Stripe/PayPal — reads only `users` + `partners`.
 */
export async function recordPartnerCommissionForSubscriber(
  clerkUserId: string
): Promise<void> {
  try {
    ...
    logger.error("recordPartnerCommissionForSubscriber failed", {
      component: "partner_commission",
      userId: clerkUserId,
