...
const ONBOARDING_NEW_RESULT_COLUMNS = `
  user_id,
  legacy_mongo_id,
  answers,
  answered_at,
...
function onboardingNewResultRowToDoc(row: OnboardingNewResultRow): AppDoc {
...
function mapRowsToDocs<T extends AppDoc = AppDoc>(rows: unknown[]): T[] {
  ...
  for (const r of rows) {
    ...
    out.push(onboardingNewResultRowToDoc(r as OnboardingNewResultRow) as T);
  }
  return out;
}

function onboardingNewResultDocToRow(doc: AppDoc, updatedAt: Date): OnboardingNewResultRow {
...
/** SQL filters for `public.onboarding_new_results`. */
export function onboardingNewResultsFilterToSql(
...
/**
 * Postgres-backed `onboarding_new_results` (one row per user; replaces `app_documents` rows).
 */
export class PgOnboardingNewResultsCollection<T extends AppDoc = AppDoc> {
  readonly collectionKey = "onboarding_new_results";

  constructor(private readonly sql: Sql) {}

  private async countAll(): Promise<number> {
    const rows =
      await this.sql`SELECT COUNT(*)::int AS c FROM public.onboarding_new_results`;
    return rows[0]?.c ?? 0;
  }

  async findDocuments(filter: Record<string, unknown>): Promise<T[]> {
    const sqlParts = onboardingNewResultsFilterToSql(filter);
    const q = new Query(filter);

    if (sqlParts && sqlParts.clause !== "true") {
      const rows = await this.sql.unsafe(
        `SELECT ${ONBOARDING_NEW_RESULT_COLUMNS} FROM public.onboarding_new_results WHERE ${sqlParts.clause} ORDER BY answered_at DESC`,
    ...
    if (n > maxScan()) {
      throw new Error(
        `Refusing to scan onboarding_new_results with ${n} rows (APP_DOCUMENTS_MAX_SCAN=${maxScan()}).`
      );
    }
    const rows = await this.sql.unsafe(
      `SELECT ${ONBOARDING_NEW_RESULT_COLUMNS} FROM public.onboarding_new_results ORDER BY answered_at DESC`,
      []
    );
    return mapRowsToDocs<T>(rows).filter((doc) => q.test(doc as never));
  }
  ...
  async countDocuments(filter: Record<string, unknown> = {}): Promise<number> {
    ...
    if (sqlParts && sqlParts.clause !== "true") {
      const rows = await this.sql.unsafe(
        `SELECT COUNT(*)::int AS c FROM public.onboarding_new_results WHERE ${sqlParts.clause}`,
        sqlParts.params as never[]
      );
      return rows[0]?.c ?? 0;
    }
  ...
  private async updateManyInternal(
    ...
    if (!matches.length) {
      if (upsert && !many) {
        ...
        const row = onboardingNewResultDocToRow(next, new Date());
        if (!row.user_id) {
        ...
        const inserted = await this.sql`
          INSERT INTO public.onboarding_new_results (
            user_id,
            legacy_mongo_id,
            answers,
            answered_at,
          ...
          ON CONFLICT (user_id) DO UPDATE SET
            legacy_mongo_id = COALESCE(
              public.onboarding_new_results.legacy_mongo_id,
    ...
    for (const doc of targets) {
      ...
      runMingoUpdate(next, update);
      const row = onboardingNewResultDocToRow(next, now);
      const answers = row.answers as Record<string, unknown>;
      await this.sql`
        UPDATE public.onboarding_new_results
        SET
