...
const HOME_AB_VARIANTS = ["classic", "passport", "legacy"] as const;
...
function normalizeVariant(value: unknown): string {
  if (typeof value === "string") {
    ...
    if ((HOME_AB_VARIANTS as readonly string[]).includes(v)) return v;
  }
  return "passport";
}
...
function homeAbEventRowToDoc(row: HomeAbEventRow): AppDoc {
  return {
...
function mapRowsToDocs<T extends AppDoc = AppDoc>(rows: unknown[]): T[] {
  ...
  for (const r of rows) {
    ...
    out.push(homeAbEventRowToDoc(r as HomeAbEventRow) as T);
  }
  return out;
}

function homeAbEventDocToRow(doc: AppDoc, mongoId: string): HomeAbEventRow {
...
/** SQL filters for `public.home_ab_events`. */
export function homeAbEventsFilterToSql(filter: Record<string, unknown>): SqlParts | null {
  ...
  for (const [k, v] of Object.entries(filter)) {
    ...
    if (k === "eventType" && typeof v === "string") {
      ...
      params.push(v);
    } else if (k === "variant" && typeof v === "string") {
      parts.push(`variant = $${paramIndex++}`);
      params.push(normalizeVariant(v));
...
/**
 * Postgres-backed `home_ab_events` collection (append-only event log).
 */
export class PgHomeAbEventsCollection<T extends AppDoc = AppDoc> {
  readonly collectionKey = "home_ab_events";

  constructor(private readonly sql: Sql) {}

  private async countAll(): Promise<number> {
    const rows = await this.sql`SELECT COUNT(*)::int AS c FROM public.home_ab_events`;
    return rows[0]?.c ?? 0;
  }

  async findDocuments(filter: Record<string, unknown>): Promise<T[]> {
    const sqlParts = homeAbEventsFilterToSql(filter);
    const q = new Query(filter);

    if (sqlParts && sqlParts.clause !== "true") {
      const rows = await this.sql.unsafe(
        `SELECT ${HOME_AB_EVENT_COLUMNS} FROM public.home_ab_events WHERE ${sqlParts.clause} ORDER BY created_at DESC`,
    ...
    if (n > maxScan()) {
      throw new Error(
        `Refusing to scan home_ab_events with ${n} rows (APP_DOCUMENTS_MAX_SCAN=${maxScan()}).`
      );
    }
    const rows = await this.sql.unsafe(
      `SELECT ${HOME_AB_EVENT_COLUMNS} FROM public.home_ab_events ORDER BY created_at DESC`,
      []
    );
    return mapRowsToDocs<T>(rows).filter((doc) => q.test(doc as never));
  }
  ...
  async insertOne(doc: unknown): Promise<{
    ...
    const row = homeAbEventDocToRow(doc as AppDoc, mongoId);
    try {
      await this.sql`
        INSERT INTO public.home_ab_events (
          mongo_id,
          event_type,
          variant,
          user_id,
  ...
  aggregate(pipeline: object[], _options?: { allowDiskUse?: boolean }): PgAggregateCursor {
    return new PgAggregateCursor(async () => {
      ...
      let docs: AppDoc[];
      if (match) {
        const sqlParts = homeAbEventsFilterToSql(match);
        if (sqlParts && sqlParts.clause !== "true") {
          const rows = await this.sql.unsafe(
            `SELECT ${HOME_AB_EVENT_COLUMNS} FROM public.home_ab_events WHERE ${sqlParts.clause} ORDER BY created_at DESC`,
            sqlParts.params as never[]
      ...
      if (n > budget) {
        throw new Error(
          `Aggregate on home_ab_events needs ${n} documents (budget ${budget}). Raise APP_AGGREGATE_DOC_BUDGET after sizing RAM.`
        );
      }
      const rows = await this.sql.unsafe(
        `SELECT ${HOME_AB_EVENT_COLUMNS} FROM public.home_ab_events ORDER BY created_at DESC`,
