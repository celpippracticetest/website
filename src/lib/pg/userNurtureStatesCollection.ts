...
const USER_NURTURE_STATE_COLUMNS = `
  user_id,
  legacy_mongo_id,
...
function userNurtureStateRowToDoc(row: UserNurtureStateRow): AppDoc {
  return {
...
function mapRowsToDocs<T extends AppDoc = AppDoc>(rows: unknown[]): T[] {
  ...
  for (const r of rows) {
    ...
    out.push(userNurtureStateRowToDoc(r as UserNurtureStateRow) as T);
  }
  return out;
}

function userNurtureStateDocToRow(doc: AppDoc, updatedAt: Date): UserNurtureStateRow {
  ...
  return {
    ...
    last_nurture_sent_at: asDate(doc.lastNurtureSentAt, null),
    nurture_flow: normalizeFlow(doc.nurtureFlow),
    nurture_stage_id:
      doc.nurtureStageId == null || doc.nurtureStageId === undefined
        ? null
        : String(doc.nurtureStageId),
    nurture_window_started_at: asDate(doc.nurtureWindowStartedAt, null),
    nurtures_in_window: asInt(doc.nurturesInWindow, 0),
    created_at: created,
    updated_at: updatedAt,
  };
}

/** SQL filters for `public.user_nurture_states`. */
export function userNurtureStatesFilterToSql(filter: Record<string, unknown>): SqlParts | null {
...
/**
 * Postgres-backed `usernurturestates` (one row per user; replaces `app_documents`).
 */
export class PgUserNurtureStatesCollection<T extends AppDoc = AppDoc> {
  readonly collectionKey = "usernurturestates";

  constructor(private readonly sql: Sql) {}

  private async countAll(): Promise<number> {
    const rows = await this.sql`SELECT COUNT(*)::int AS c FROM public.user_nurture_states`;
    return rows[0]?.c ?? 0;
  }

  async findDocuments(filter: Record<string, unknown>): Promise<T[]> {
    const sqlParts = userNurtureStatesFilterToSql(filter);
    ...
    if (n > maxScan()) {
      throw new Error(
        `Refusing to scan user_nurture_states with ${n} rows (APP_DOCUMENTS_MAX_SCAN=${maxScan()}).`
      );
    }
    const rows = await this.sql.unsafe(
      `SELECT ${USER_NURTURE_STATE_COLUMNS} FROM public.user_nurture_states ORDER BY updated_at DESC`,
      []
    );
    return mapRowsToDocs<T>(rows).filter((doc) => q.test(doc as never));
  }
  ...
  async insertOne(doc: unknown): Promise<{
    ...
    if (!d.userId) {
      throw new Error("usernurturestates requires userId");
    }
    const row = userNurtureStateDocToRow(d, new Date());
    if (!row.legacy_mongo_id) {
      row.legacy_mongo_id = mongoId;
    }
    try {
      await this.sql`
        INSERT INTO public.user_nurture_states (
          user_id,
          legacy_mongo_id,
          last_nurture_sent_at,
          nurture_flow,
        ...
        VALUES (
          ${row.user_id},
          ${row.legacy_mongo_id},
          ${row.last_nurture_sent_at},
          ${row.nurture_flow},
          ${row.nurture_stage_id},
          ${row.nurture_window_started_at},
          ${row.nurtures_in_window},
          ${row.created_at},
          ${row.updated_at}
        )
      `;
  ...
  private async updateManyInternal(
    ...
    if (!matches.length) {
      if (upsert && !many) {
        ...
        const row = userNurtureStateDocToRow(next, new Date());
        if (!row.user_id) {
    ...
    for (const doc of targets) {
      ...
      runMingoUpdate(next, update);
      const row = userNurtureStateDocToRow(next, now);
      await this.sql`
        UPDATE public.user_nurture_states
        SET
          last_nurture_sent_at = ${row.last_nurture_sent_at},
          nurture_flow = ${row.nurture_flow},
          nurture_stage_id = ${row.nurture_stage_id},
          nurture_window_started_at = ${row.nurture_window_started_at},
          nurtures_in_window = ${row.nurtures_in_window},
          updated_at = ${now}
  ...
  async createIndex(
    ...
    return "noop_pg_user_nurture_states_index";
  }
}
