...
const PRICING_MODEL_AB_VARIANTS = ["period", "weekly_equiv"] as const;

type PricingModelAbEventRow = {
  mongo_id: string;
  event_type: string;
  model: string;
  user_id: string | null;
...
const PRICING_MODEL_AB_EVENT_COLUMNS = `
  mongo_id,
  event_type,
  model,
  user_id,
...
function normalizeModel(value: unknown): string {
  if (typeof value === "string") {
    ...
    if ((PRICING_MODEL_AB_VARIANTS as readonly string[]).includes(v)) return v;
  }
  return "period";
}
...
function pricingModelAbEventRowToDoc(row: PricingModelAbEventRow): AppDoc {
  return {
...
function mapRowsToDocs<T extends AppDoc = AppDoc>(rows: unknown[]): T[] {
  ...
  for (const r of rows) {
    ...
    out.push(pricingModelAbEventRowToDoc(r as PricingModelAbEventRow) as T);
  }
  return out;
}

function pricingModelAbEventDocToRow(doc: AppDoc, mongoId: string): PricingModelAbEventRow {
...
/** SQL filters for `public.pricing_model_ab_events`. */
export function pricingModelAbEventsFilterToSql(
  ...
  for (const [k, v] of Object.entries(filter)) {
    ...
    if (k === "eventType" && typeof v === "string") {
      ...
      params.push(v);
    } else if (k === "model" && typeof v === "string") {
      parts.push(`model = $${paramIndex++}`);
      params.push(normalizeModel(v));
...
/** Postgres-backed `pricing_model_ab_events` collection (append-only event log). */
export class PgPricingModelAbEventsCollection<T extends AppDoc = AppDoc> {
  readonly collectionKey = "pricing_model_ab_events";

  constructor(private readonly sql: Sql) {}

  private async countAll(): Promise<number> {
    const rows =
      await this.sql`SELECT COUNT(*)::int AS c FROM public.pricing_model_ab_events`;
    return rows[0]?.c ?? 0;
  }

  async findDocuments(filter: Record<string, unknown>): Promise<T[]> {
    const sqlParts = pricingModelAbEventsFilterToSql(filter);
    const q = new Query(filter);

    if (sqlParts && sqlParts.clause !== "true") {
      const rows = await this.sql.unsafe(
        `SELECT ${PRICING_MODEL_AB_EVENT_COLUMNS} FROM public.pricing_model_ab_events WHERE ${sqlParts.clause} ORDER BY created_at DESC`,
    ...
    if (n > maxScan()) {
      throw new Error(
        `Refusing to scan pricing_model_ab_events with ${n} rows (APP_DOCUMENTS_MAX_SCAN=${maxScan()}).`,
      );
    }
    const rows = await this.sql.unsafe(
      `SELECT ${PRICING_MODEL_AB_EVENT_COLUMNS} FROM public.pricing_model_ab_events ORDER BY created_at DESC`,
      [],
    );
    return mapRowsToDocs<T>(rows).filter((doc) => q.test(doc as never));
  }
  ...
  async insertOne(doc: unknown): Promise<{
    ...
    const row = pricingModelAbEventDocToRow(doc as AppDoc, mongoId);
    try {
      await this.sql`
        INSERT INTO public.pricing_model_ab_events (
          mongo_id,
          event_type,
          model,
          user_id,
  ...
  aggregate(pipeline: object[], _options?: { allowDiskUse?: boolean }): PgAggregateCursor {
    return new PgAggregateCursor(async () => {
      ...
      let docs: AppDoc[];
      if (match) {
        const sqlParts = pricingModelAbEventsFilterToSql(match);
        if (sqlParts && sqlParts.clause !== "true") {
          const rows = await this.sql.unsafe(
            `SELECT ${PRICING_MODEL_AB_EVENT_COLUMNS} FROM public.pricing_model_ab_events WHERE ${sqlParts.clause} ORDER BY created_at DESC`,
            sqlParts.params as never[],
      ...
      if (n > budget) {
        throw new Error(
          `Aggregate on pricing_model_ab_events needs ${n} documents (budget ${budget}). Raise APP_AGGREGATE_DOC_BUDGET after sizing RAM.`,
        );
      }
      const rows = await this.sql.unsafe(
        `SELECT ${PRICING_MODEL_AB_EVENT_COLUMNS} FROM public.pricing_model_ab_events ORDER BY created_at DESC`,
