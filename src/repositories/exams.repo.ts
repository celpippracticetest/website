...
import { ExamSchema, ExamSchemaDto, TExamSchema, TExamSchemaDto } from "@/models/exam.model";
...
export class ExamRepository {
  ...
  private convertFromEntity(taskEntity: TExamSchema) {
    ...
    async findExamById(id: string): Promise<TExamSchemaDto | null> {
      const entity = await this.getExamCollection().findOne({ _id: new ObjectId(id) });
  ...
  async getAllExam(
    ...
    page: number = 0,
    limit: number = 10
  ): Promise<{ items: TExamSchemaDto[]; hasNextPage: boolean; page: number; totalPages: number; totalItems: number }> {
    ...
    const results = await this.getExamCollection().aggregate(aggregateFilter).toArray();
    ...
    return {
      items: practices.map((practice: TExamSchema) => this.convertFromEntity(practice)),
      page,
