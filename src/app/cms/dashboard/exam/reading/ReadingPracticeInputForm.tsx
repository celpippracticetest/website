...
export default function ReadingPracticeInputForm() {
  ...
  async function uploadFile(file: File): Promise<string | null> {
    ...
    const upload = new Upload({
      ...
      params: {
        ...
        Key: `exams/reading/${Date.now()}_${file.name}`, // Unique key for the file
        Body: file,
      },
    });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const result = await saveReadingPractice(formData, selectedExamId);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h1 className="text-2xl font-bold">Create Reading Exam</h1>
        <p className="text-muted-foreground">
          Fill in the form below to create a new reading exam exercise.
        </p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Processing..." : "Create Reading Exam"}
            </Button>
          </div>
        </form>
      </Form>
