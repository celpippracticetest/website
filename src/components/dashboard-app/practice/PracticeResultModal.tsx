...
const PracticeResultModal = ({
  ...
  const [activeTab, setActiveTab] = React.useState<
    "feedback" | "improvements" | "betterVersion"
  >("feedback");
  ...
  const scoreDetails = getScoreDescription(
    data?.result ? data.result.overall : 0,
  );
        ...
        <div className="text-[14px] text-muted-foreground border-slate-300 p-6 pb-28 flex flex-col gap-4">
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden bg-white mb-2">
            ...
            >
              ...
              <SvgChevronDown
                className={`w-5 h-5 text-slate-500 transition-transform duration-200 ${isTaskResponseOpen ? "rotate-180" : ""}`}
              />
            </button>
            ...
            {isTaskResponseOpen && (
              <div className="p-4 pt-0 border-t border-slate-100 flex flex-col gap-4">
                ...
                <div
                  className={`${taskContent ? "mt-0" : "mt-4"} flex flex-col gap-2`}
                >
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-col min-[744px]:flex-row gap-4 mb-6 justify-center sticky top-0 z-10 bg-white/95 backdrop-blur-sm p-4 -mx-4 border-b border-sidebar-border/50">
              <button
                ...
                className={`px-6 py-2.5 w-full rounded-[8px] text-[14px] font-medium transition-all duration-200 text-[#316BFF] ${
                  activeTab === "feedback"
                    ? "bg-[#D6E6FF]"
              ...
              <button
                ...
                className={`px-6 py-2.5 w-full rounded-[8px] text-[14px] font-medium transition-all duration-200 text-[#F27059] ${
              ...
              <button
                ...
                className={`px-6 py-2.5 w-full rounded-[8px] text-[14px] font-medium transition-all duration-200 text-[#0DAA94] ${
            ...
            <div className="min-h-[200px]">
              ...
              <div
                id="feedback"
                className="flex flex-col gap-2 mb-10 pt-4 scroll-mt-24"
              >
                ...
                <div className="prose prose-base max-w-none prose-blue text-slate-700 bg-[#F0F6FF] p-6 rounded-[12px] leading-relaxed">
                  <InteractiveText
                    text={(data?.result?.feedback || "").replace(
                      /<\/?feedback>/g,
                      "",
                    )}
                    onAskAI={askAboutWord}
              ...
              <div
                id="improvements"
                className="flex flex-col gap-3 mb-10 pt-4 scroll-mt-24"
              >
                ...
                <div className="prose prose-base max-w-none text-slate-800 bg-[#FFF5EF] p-8 rounded-[12px] leading-8 text-[16px]">
                  <span>
                    {(() => {
                      ...
                      const originalText =
                        data?.text ||
                        data?.result?.grammarMistakes
                          ?.map((m: { original: string }) => m.original)
                          .join(" ") ||
                        "";
                        ...
                        return diff.map((part, index) => {
                          ...
                          if (part.added) {
                            if (onlyShowCorrect) {
                              return (
                                <span
                                  key={index}
                                  className="bg-[#B3FFBD] text-slate-900 mx-0.5 px-0.5 rounded-[2px] font-normal"
                                >
                                  {part.value}
                                </span>
                              );
                          ...
                          if (part.removed) {
                            ...
                            return (
                              <span
                                key={index}
                                className="bg-[#FFBDB3] text-slate-900 mx-0.5 px-0.5 rounded-[2px] line-through decoration-slate-500/50"
                              >
                                {part.value}
                              </span>
                            );
                ...
                <div className="flex flex-col gap-0 text-slate-800 bg-[#F2FFFD] rounded-[12px] overflow-hidden">
                  ...
                  <div className="flex items-center justify-center gap-2 p-6 pb-2 pt-8">
                    ...
                    >
                      ...
                      <span className="text-[14px] font-bold text-slate-900 underline decoration-[#0DAA94] decoration-2 underline-offset-4">
                        Click to save all words to learn them later.
                      </span>
                    </button>
                  </div>
