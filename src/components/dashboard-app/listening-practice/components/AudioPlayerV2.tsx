...
export default function AudioPlayerV2({
    ...
    if (isTTS) {
      ...
      if (isPlaying) {
        ...
        void audio.play().catch((err: unknown) => {
          trackKpi.audioError({
            clipId: audioUrl,
            errorType: err instanceof Error ? err.name : "play_rejected",
            browser: navigator.userAgent,
          });
        });
      }
  ...
  const progressPercent = isTTS
    ? textToSpeak && textToSpeak.length > 0
      ? (currentTime / textToSpeak.length) * 100
      : 0
    : duration
  ...
  return (
    <div
      className={`flex items-center gap-6 border border-[#0DAA94] rounded-full px-5 py-3 bg-white w-full max-w-[600px] ${className}`}
    >
      <button
        onClick={togglePlay}
      ...
      <div
        className={`relative w-full h-4 flex items-center ${isTTS ? "cursor-not-allowed" : ""}`}
      >
        <input
          type="range"
          min={0}
